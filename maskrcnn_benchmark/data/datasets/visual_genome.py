...
class VGDataset(torch.utils.data.Dataset):
    ...
    def get_statistics(self):
        ...
        fg_matrix, bg_matrix,stat_classes = get_VG_statistics(img_dir=self.img_dir, roidb_file=self.roidb_file, dict_file=self.dict_file,
                                                image_file=self.image_file, must_overlap=True)
...
def get_VG_statistics(img_dir, roidb_file, dict_file, image_file, must_overlap=True):
    ...
    # train_data = VGDataset(split='test', img_dir=img_dir, roidb_file=roidb_file,
    #                     dict_file=dict_file, image_file=image_file, num_val_im=5000,
    #                     filter_duplicate_rels=False)
    ...
    stat_relation= np.zeros((num_rel_classes), dtype=np.int64)

    stat_classes = np.zeros((num_obj_classes), dtype=np.int64)
    ...
    return fg_matrix, bg_matrix,stat_classes
