...
def do_vg_evaluation(
    ...
    zeroshot_triplet = torch.load("maskrcnn_benchmark/data/datasets/evaluation/vg/zeroshot_triplet_new.pytorch")
    ...
    num_rel_category = cfg.MODEL.ROI_RELATION_HEAD.VG_NUM_CLASSES
    ...
    result_str = '\n' + '=' * 100 + '\n'
    if "bbox" in iou_types:#detection————和SGG无关
    ...
    if "relations" in iou_types:#######初始化
        result_dict = {}
        evaluator = {}
        # tradictional Recall@K
        eval_recall = SGRecall(result_dict)
        eval_recall.register_container(mode)#sgdet
        evaluator['eval_recall'] = eval_recall#initialize（20，50，100）
        ...
        eval_mean_recall = SGMeanRecall(result_dict, num_rel_category, dataset.ind_to_predicates,dataset.ind_to_classes, print_detail=True)
        ...
        for groundtruth, prediction in zip(groundtruths, predictions):
            evaluate_relation_of_one_image(groundtruth, prediction, global_container, evaluator)##############计算recall
    ...
    if "relations" in iou_types:
        if output_folder:
            ...
            torch.save(result_dict, os.path.join(output_folder,myoutfold, 'result_dict.pytorch'))
...
def evaluate_relation_of_one_image(groundtruth, prediction, global_container, evaluator):
    ...
    local_container = evaluator['eval_recall'].calculate_recall(global_container, local_container, mode)################计算recall
    ...
    # Mean Recall
    evaluator['eval_mean_recall'].collect_mean_recall_items(global_container, local_container, mode)################计算meanrecall
