...
class CombinedROIHeads(torch.nn.ModuleDict):
    ...
    def __init__(self, cfg, heads):
        ...
        # self.proj_CLIP = nn.Linear(256+151, 256)

    def forward(self, features, proposals, targets=None, logger=None):
        losses = {}
        # B, C, H, W = features[3].shape
        # x_out = torch.cat([features[3], fusion_feat], dim=1)
        # x_out=x_out.reshape(B, -1, H*W).permute(2, 0, 1)#HW,2,C
        # x_out = self.proj_CLIP(x_out).permute(1, 2, 0)

        # DenseCLIP-addroibox
        # features = list(features)
        # features[3] = x_out.reshape(B, -1, H, W)
