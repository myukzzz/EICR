...
class BDC(nn.Module):
    def __init__(self, is_vec=True, input_dim=640, dimension_reduction=None, activate='relu'):
        ...
        if self.dr is not None and self.dr != self.input_dim:
            ...
            # self.act
            # )
            self.conv_dr_block = nn.Linear(self.input_dim, self.dr)
        ...
        self.temperature = nn.Parameter(torch.log((1. / (2 * 80*80)) * torch.ones(1,1)), requires_grad=True)
