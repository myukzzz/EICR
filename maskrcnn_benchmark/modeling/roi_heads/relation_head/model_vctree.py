...
class VCTreeLSTMContext(nn.Module):
    ...
    def forward(self, x, proposals, rel_pair_idxs, logger=None, all_average=False, ctx_average=False,return_pos=False):
        ...
        if self.cfg.MODEL.ROI_RELATION_HEAD.USE_GT_OBJECT_LABEL:#precl/sgcl
        ...
        pos_embed = self.pos_embed(box_info)#linear-BN-linear-RELU——9-128
        ...
        if all_average and self.effect_analysis and (not self.training):
            ...
            obj_pre_rep = cat((x, obj_embed, pos_embed), -1)#obj_pre_rep-(B*N)*(4096+128+200[objemb])

        # construct VCTree
        box_inp = self.box_embed(box_info)#Linear-BN-RELU-9-128，bbox相关
        pair_inp = self.overlap_embed(get_overlap_info(proposals))#Linear-BN-RELU-6-128，bbox相关
        bi_inp = cat((self.obj_reduce(x.detach()), self.emb_reduce(obj_embed.detach()), box_inp, pair_inp), -1)#ROIfeat和objemb统一归到128dim后，cat+128*2——512
        bi_preds, vc_scores = self.vctree_score_net(num_objs, bi_inp, obj_logits, proposals)#bi_preds,没用，都是0
    ...
    def vctree_score_net(self, num_objs, roi_feat, roi_dist, proposals):
        roi_dist = roi_dist.detach()#roi_dist-(B*N)*151,目标维度为1000其余为-1000;roi_feat-(B*N)*512
        roi_dist = F.softmax(roi_dist, dim=-1)#roi_dist转化0-1 onehot
        # separate into each image
        roi_feat = F.relu(self.score_pre(roi_feat))#linear-RELU-512-512
        ...
        for sub, obj, dist, prp in zip(sub_feats, obj_feats, roi_dists, proposals):
            ...
            num_obj = sub.shape[0]
            num_dim = sub.shape[-1]
            sub = sub.view(1, num_obj, num_dim).expand(num_obj, num_obj, num_dim)#N*1
            obj = obj.view(num_obj, 1, num_dim).expand(num_obj, num_obj, num_dim)#1*N
            ...
            joint_dist = (sub_dist * obj_dist).view(num_obj, num_obj, -1)#joint_dist-N*N*(151*151)——主宾的embedding编码？
            
            co_prior = self.bi_freq_prior(joint_dist.view(num_obj*num_obj, -1)).view(num_obj, num_obj)#N*N*1
            vis_prior = self.vision_prior(cat([sub * obj, sub, obj, co_prior.unsqueeze(-1)], dim=-1).view(num_obj*num_obj, -1)).view(num_obj, num_obj)#s.*o和s和o和emb编码——512*3+1;visprior-linear-1537-1
