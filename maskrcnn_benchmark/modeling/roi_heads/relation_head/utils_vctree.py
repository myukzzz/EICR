...
def generate_forest(pair_scores, proposals, mode):#pair_scores-B*(N*N)*1，区分s，o直接是否存在关系
    """
    ...
    for pair_score, proposal in zip(pair_scores, proposals):
        ...
        root_idx = int(node_scores.max(-1)[1])#可能关联性最多的节点作为root

        root = ArbitraryTree(root_idx, float(node_scores[root_idx]), int(obj_label[root_idx]), proposal.bbox[root_idx], is_root=True)#树节点
...
def gen_tree(node_container, pair_score, node_scores, root, remain_index, mode):
    ...
    while len(node_container) > 0:
        ...
        best_id = select_score_map.max(0)[1]#寻找当前root的最大联系score作为子节点

        depend_id = int(best_id) // wid#父节点的index
        insert_id = int(best_id) % wid#select里的index
        best_depend_node = select_node[depend_id]
        best_insert_node = node_container[insert_id]
        best_depend_node.add_child(best_insert_node)#插入子节点
