...
class ROIRelationHead(torch.nn.Module):
    ...
    def forward(self, features, proposals, targets=None, logger=None):
        ...
        if self.training:
            ...
            rel_pair_idxs = self.samp_processor.prepare_test_pairs(features[0].device, proposals)#6320
        #proposal-nograd
        #roifeat-head
        # use box_head to extract features that will be fed to the later predictor processing
        roi_features = self.box_feature_extractor(features, proposals)#ROIalign-8*80 256 7 7——8*80 4096
        ...
        if self.use_union_box:
            union_features = self.union_feature_extractor(features, proposals, rel_pair_idxs)#all_pairs
        else:
            union_features = None#all-pairs 4096
