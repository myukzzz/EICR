...
class LevelMapper(object):
    ...
    def __call__(self, boxlists):
        ...
        # Eqn.(1) in FPN paper
        target_lvls = torch.floor(self.lvl0 + torch.log2(s / self.s0 + self.eps))#floor-最小整数
        target_lvls = torch.clamp(target_lvls, min=self.k_min, max=self.k_max)#torch.clamp 输入input张量每个元素的夹紧到区间 [min,max][min,max]——按框大小分组
...
class Pooler(nn.Module):
    ...
    def forward(self, x, boxes):
        ...
        #收集总proposals
        rois = self.convert_to_roi_format(boxes)#640*[4+index]
